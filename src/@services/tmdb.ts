...
import {
  ...
  Credits,
  PaginatedResponse,
} from "@/@types/tmdb"; // Adjust imports as needed
...
async function fetcher<T>(
  ...
  try {
    ...
    if (!res.ok) {
      ...
      console.error(
        `[TMDB] Error ${res.status} for ${sanitizedUrl}:`,
    ...
    console.error(`[TMDB] Network error for ${sanitizedUrl}:`, err);
...
export async function getPopularMoviesByGenre(
  genreId: number,
  page?: number,
): Promise<PaginatedResponse<Movie>> {
...
export async function getSimilarMovies(
  ...
  return fetcher<PaginatedResponse<Movie>>(`/movie/${movieId}/similar`, {
    page,
  });
}
